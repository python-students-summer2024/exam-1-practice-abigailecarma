...
def get_guess(max_int):
    user_guess = input(f"Guess a random integer between 1 and {max_int}, inclusive: ")
    rand = get_random_int(1, max_int)
    if not user_guess.isdigit() or int(user_guess) < 1 or int(user_guess) > max_int:
        return int(-1)
    elif int(user_guess) == rand:
        return True
    elif int(user_guess) != rand:
        return False
...
def play_game():
    ...
    for x in range(4):
        ...
        if guess == -1:
            ...
            guess_dec = guess_dec + 0.25
            print("Correct!")
        elif guess == False:
            print( "Wrong!")

    guess_percent = format(guess_dec, '.0%')
    print(f"You guessed {guess_percent} of the random numbers correctly.") 
